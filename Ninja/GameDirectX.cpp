...
{
	HRESULT hr = d3ddv->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &backBuffer);
	return !FAILED(hr);
}
...
void GameDirectX::PrintText(char* str, int size, int x, int y, DWORD color)
{
	...
	D3DXCreateFontA(d3ddv, size, 0, FW_NORMAL, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
		"Arial", &dxfont);
	dxfont->DrawTextA(NULL, str, strlen(str), &textbox, DT_LEFT | DT_TOP, color);
	dxfont->Release();
}
